...
if [ "$peers" ]; then
        ...
        do
                ...
                if [ "$peer_verified" ]; then
                        ...
                        if [ "$status" = "OK" ]; then
# MTT
                                status2=`echo "$command_output" | grep "^[[:space:]]*Status[[:space:]]*:" | awk '{print $4 $5;}' | sed 's,(,,; s,ms),,'`
                                if [ "$test_ok" ]; then
                                        test_ok="${test_ok}, $p ($status2 ms)"
#                                       test_ok="${test_ok} $p"
                                else
                                        test_ok="$p ($status2 ms)"
#                                       test_ok="$p"

                                fi
#
                                if [ $status2 -gt $MTT_WARN ]; then
                                        test_warn="WARN: $p ($status2 ms)"
                                fi
                                if [ $status2 -gt $MTT_CRIT ]; then
                                        test_errors="$p ($status2 ms)"
                                fi
                        else
                                if [ "$status" ]; then
                                        status_error="$status"
